  ...
  module Admin
    class ProductsController < ResourceController
      ...
      def add_cart
        ...
        order = Order.new
        order.assign_attributes(Order.first.as_json.except("id", "created_at", "updated_at"))
        order.number = SecureRandom.alphanumeric(10)
        order.token = SecureRandom.uuid
        variant.sku =~ /(\d+\.\d+)$/
        total = $1
        order.total = total.present? ? total.to_i : rand(70..100) / 100.00
        respond_to do |format|
          format.html
          format.js do
            if order.save
              render js: "alert('Add Cart Success!');"
            else
              render js: "alert('Add Cart Error!');"
            end
          end
        end
      end
